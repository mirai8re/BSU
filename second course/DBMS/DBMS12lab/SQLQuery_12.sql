...
-- Табличные переменные объявляются с помощью оператора DECLARE и имеют префикс @. 
-- Они видны только в соединении, которое их создает, и удаляются при завершении пакета или хранимой процедуры. 
-- Их можно использовать в качестве параметров для хранимых процедур или функций, но они не могут быть индексированы или иметь ограничения. 
-- Например:
DECLARE @t TABLE (id INT, name VARCHAR(50));
INSERT INTO @t VALUES (1, 'Alice'), (2, 'Bob');
SELECT * FROM @t;
----------------
-- Временные локальные таблицы создаются с помощью оператора CREATE TABLE и имеют префикс #. 
-- Они видны только для создающего их соединения и удаляются при закрытии соединения. 
-- Они могут быть индексированы и иметь ограничения, но не могут использоваться в качестве параметров хранимых процедур или функций. 
-- Например:
CREATE TABLE #t (id INT, name VARCHAR(50));
INSERT INTO #t VALUES (1, 'Alice'), (2, 'Bob');
SELECT * FROM #t;
----------------
-- Временные глобальные таблицы создаются с помощью оператора CREATE TABLE и имеют префикс ##. 
-- Они видны всем и удаляются после закрытия всех соединений, которые на них ссылались. 
-- Они могут индексироваться и иметь ограничения, но не могут использоваться в качестве параметров для хранимых процедур или функций. 
-- Например:
CREATE TABLE ##t (id INT, name VARCHAR(50));
INSERT INTO ##t VALUES (1, 'Alice'), (2, 'Bob');
SELECT * FROM ##t;
----------------
-- Общие табличные выражения (CTE) определяются с помощью оператора WITH и имеют имя без префикса. 
-- Они действительны только для следующего оператора и могут использоваться для упрощения сложных запросов или рекурсивных запросов. 
-- Они могут ссылаться на другие CTE в том же выражении WITH, но не могут быть индексированными или иметь ограничения. 
-- Например:
-- Получение списка всех клиентов и их адресов
use PizzaDB
WITH CustomerAddresses AS (
SELECT Customers.CustomerID, Customers.FirstName, Customers.LastName, Addresses.Street, Addresses.City, Addresses.State, Addresses.ZipCode
FROM Customers
